...
export function FormStorageManager({ isOpen, onClose }: FormStorageManagerProps) {
  const { getAllSubmissions, deleteSubmission, updateSubmissionStatus, userProfile, getFormStatistics, exportAllData } = useFormStorage('manager');
  const [searchQuery, setSearchQuery] = useState('');
  const [filterType, setFilterType] = useState('all');
  const [filterStatus, setFilterStatus] = useState('all');
  const [selectedSubmission, setSelectedSubmission] = useState<any>(null);
  const [showDetails, setShowDetails] = useState(false);
  const [activeTab, setActiveTab] = useState<'submissions' | 'profile' | 'statistics'>('submissions');

  const allSubmissions = getAllSubmissions();
  const statistics = getFormStatistics();

  const filteredSubmissions = allSubmissions.filter(submission => {
    const matchesSearch = submission.formType.toLowerCase().includes(searchQuery.toLowerCase()) ||
                         submission.formTitle.toLowerCase().includes(searchQuery.toLowerCase()) ||
                         JSON.stringify(submission.data).toLowerCase().includes(searchQuery.toLowerCase()) ||
                         (submission.userInfo?.name || '').toLowerCase().includes(searchQuery.toLowerCase()) ||
                         (submission.userInfo?.email || '').toLowerCase().includes(searchQuery.toLowerCase());
  ...
  const getFormIcon = (formType: string) => {
    ...
    if (formType.includes('video')) {
      return <Video className="w-4 h-4" />;
    }
    if (formType.includes('user') || formType.includes('profile')) {
                    ...
                    {filteredSubmissions.map((submission, index) => (
                      ...
                      >
                        ...
                        <div className="text-xs text-gray-500 dark:text-gray-400 space-y-1">
                          ...
                          <div>
                            {Object.keys(submission.data).length} fields captured
                          </div>
                        </div>
                      </motion.div>
                    ))}
